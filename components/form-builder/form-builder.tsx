...
import PaymentFieldEditor from './payment-field-editor';
...
export default function FormBuilder({ formName, formType, formId: initialFormId }: FormBuilderProps) {
  ...
  const addField = (fieldType: string) => {
    const newField = {
      ...
      helpText: '',
      defaultValue: '',
      paymentSettings: fieldType === 'payment'
        ? {
            provider: 'paystack',
            amount: 0,
            currency: 'NGN',
      ...
      const response = await fetch('/api/forms/save', {
        ...
        body: JSON.stringify({
          ...
          fields: fields.map((f, i) => ({
            ...
            options: f.type === 'payment' ? { paymentSettings: f.paymentSettings } : f.options,
            validations: f.validations,
            conditionalLogic: f.conditionalLogic,
            order: i,
          })),
          settings: { ...formSettings, formType },
        }),
      });
      ...
      <div className="w-96 bg-white border-l border-gray-200 overflow-y-auto">
        {selectedField ? (
            ...
            <div className="border-t pt-4">
              {selectedField.type === 'payment' ? (
                <PaymentFieldEditor
                  ...
                  settings={selectedField.paymentSettings || {}}
                  onChange={(paymentSettings) => updateField(selectedField.id, { paymentSettings })}
                />
              ) : (
                <FieldOptionsEditor
                  fieldId={selectedField.id}
          ...
          <div className="p-4">
            ...
            <FormSettingsAdvanced formSettings={formSettings} onSettingsChange={setFormSettings} />
          </div>
        )}
