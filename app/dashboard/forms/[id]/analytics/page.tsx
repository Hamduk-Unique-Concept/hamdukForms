...
function AnalyticsPageContent({ formId }: { formId: string }) {
  ...
  useEffect(() => {
    ...
    const fetchAnalytics = async () => {
      try {
        ...
        const response = await fetch(
          `/api/forms/${formId}/analytics?period=${period}`,
    ...
    fetchAnalytics();
  }, [formId, period, session?.access_token]);
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card className="p-6">
          ...
          <div className="text-3xl font-bold mt-2">{analytics.completionRate}%</div>
          <div className="text-xs text-gray-500 mt-2">completed submissions</div>
        </Card>
      </div>
            ...
            <tbody>
              {analytics.fieldAnalytics.map((field, idx) => (
                <tr key={idx} className="border-b hover:bg-gray-50">
                  ...
                  <td className="p-3 text-orange-600">0%</td>
                </tr>
              ))}
            </tbody>
          </table>
...
export default function AnalyticsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <FeatureGate featureKey="advanced_analytics" featureName="Advanced Analytics">
      <AnalyticsPageContent formId={id} />
    </FeatureGate>
  );
}
