...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const seatAccess = await checkFeatureAccess(organizationId, 'team_seats');
    if (!seatAccess.allowed) {
      return NextResponse.json(
        {
          error: 'PLAN_LIMIT_REACHED',
          feature: 'team_seats',
          limit: seatAccess.limit,
          usage: seatAccess.usage,
          message: 'Your current plan has reached its team seat limit.',
        },
        { status: 403 }
      );
    }
